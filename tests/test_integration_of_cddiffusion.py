...
"""
test_integration_of_cddiffusion
----------------------------------

Integration tests for `cddiffusion` module.
"""
...
SKIP_REASON = 'CDDIFFUSION_DOCKER_IMAGE, CDDIFFUSION_DOCKER, CDDIFFUSION_TMPDIR environment ' \
              'variable(s) not set to a docker image' \
              ' cannot run integration tests of hidef with Docker'


@unittest.skipUnless(os.getenv('CDDIFFUSION_DOCKER_IMAGE') is not None and
                     os.getenv('CDDIFFUSION_DOCKER') is not None and
                     os.getenv('CDDIFFUSION_TMPDIR') is not None, SKIP_REASON)
class TestCdDiffusionInDocker(unittest.TestCase):
    ...
    def run_hidef_docker(self, cmdargs, temp_dir=None):
        ...
        :rtype: tuple
        """
        cmd = [os.getenv('CDDIFFUSION_DOCKER'), 'run', '--rm', '-v',
               temp_dir+':'+temp_dir,
               os.getenv('CDDIFFUSION_DOCKER_IMAGE')]
        cmd.extend(cmdargs)
    ...
    def test_run_hidef_no_args(self):
        temp_dir = tempfile.mkdtemp(dir=os.getenv('CDDIFFUSION_TMPDIR'))
        try:
            src_input_file = os.path.join(TestCdDiffusionInDocker.HUNDRED_NODE_DIR, 'input.txt')
